...
public class SecureCC {
    ...
    @Mod.Instance(SecureCC.MODID)
    public static SecureCC instance;
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        // Friend-management GUI packets.
        SecureNetwork.init();
        // Secure cable and secure modem: expose the "secure_modem"
        // peripheral to adjoining computers. Must be in preInit, before CC
        // queries providers.
        if (SecureConfig.enableCable) {
            ComputerCraftAPI.registerPeripheralProvider(new IPeripheralProvider() {
                @Override
                public IPeripheral getPeripheral(World world, BlockPos pos, EnumFacing side) {
                    if (world.getBlockState(pos).getBlock() != ModBlocks.SECURE_CABLE) return null;
                    if (ModBlocks.SECURE_CABLE instanceof IPeripheralProvider) {
                        return ((IPeripheralProvider) ModBlocks.SECURE_CABLE).getPeripheral(world, pos, side);
                    }
                    return null;
                }
            });
        }
        if (SecureConfig.enableModem) {
            ComputerCraftAPI.registerPeripheralProvider(new IPeripheralProvider() {
                @Override
                public IPeripheral getPeripheral(World world, BlockPos pos, EnumFacing side) {
                    if (world.getBlockState(pos).getBlock() != ModBlocks.SECURE_MODEM) return null;
                    if (ModBlocks.SECURE_MODEM instanceof IPeripheralProvider) {
                        return ((IPeripheralProvider) ModBlocks.SECURE_MODEM).getPeripheral(world, pos, side);
                    }
                    return null;
                }
            });
    ...
    public void init(FMLInitializationEvent event) {
        // Friend-management GUI. The screen is containerless on the client,
        // but the server side returns a dummy Container so Forge actually
        // sends the OpenGui packet (it skips null server GUI elements).
        NetworkRegistry.INSTANCE.registerGuiHandler(instance, new SecureGuiHandler());
        ...
        if (tab != null) {
            for (Item item : new Item[]{ModItems.SECURE_COMPUTER, ModItems.SECURE_TURTLE,
                    ...
                    ModItems.SECURE_CABLE, ModItems.SECURE_MODEM,
                    ModItems.SECURE_NEURAL_INTERFACE, ModItems.SECURITY_KEY}) {
                item.setCreativeTab(tab);
            }
        }
        // Security Key recipe lives in assets/securecc/recipes/security_key.json
        // (uses computercraft:disk_expanded, the craftable floppy disk).
    }
